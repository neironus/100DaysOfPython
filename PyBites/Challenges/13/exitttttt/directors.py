...
def print_results(directors):
    ...
    sep_line = '-' * 60
    report = sorted(directors.items(), key=lambda x: (float(x[0][1]), x[0][0] ), reverse=True)
    i = 0
    for director, movies in report:
        i += 1
        if i > NUM_TOP_DIRECTORS:
            continue

        print(fmt_director_entry.format(counter=i, director=director[0], avg=director[1]))
        print(sep_line)
        for movie in movies:
